// OllaForge - A web application that simplifies training LLMs with your own data for use in Ollama.
// Copyright (C) 2026  Marcel Joachim Kloubert (marcel@kloubert.dev)
//
// This program is free software: you can redistribute it and/or modify
// it under the terms of the GNU Affero General Public License as
// published by the Free Software Foundation, either version 3 of the
// License, or (at your option) any later version.
//
// This program is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU Affero General Public License for more details.
//
// You should have received a copy of the GNU Affero General Public License
// along with this program.  If not, see <https://www.gnu.org/licenses/>.

import { useMemo } from "react";

import { useLLMProviders } from "@/hooks/useLLMProviders";
import type { LLMProviderStatus } from "@/types";

interface UseConfiguredProvidersResult {
  isLoading: boolean;
  configuredProviders: LLMProviderStatus[];
  hasConfiguredProviders: boolean;
  refresh: () => Promise<void>;
}

/**
 * Hook to get only configured (valid) LLM providers.
 * Filters providers that have valid: true.
 */
export function useConfiguredProviders(): UseConfiguredProvidersResult {
  const { isLoading, providers, refresh } = useLLMProviders();

  const configuredProviders = useMemo(() => {
    return providers.filter((p) => p.valid);
  }, [providers]);

  const hasConfiguredProviders = configuredProviders.length > 0;

  return {
    isLoading,
    configuredProviders,
    hasConfiguredProviders,
    refresh,
  };
}
